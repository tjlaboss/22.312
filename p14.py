...
def x_iterate(t, xguess = X0, Tguess = 400, eps = 0.02):
	...
	for Tguess in range(Tmin, Tmax, 5):
		...
		converged = True
		overflooded = False
		while abs(xguess - last_x)/xguess > eps or abs(u_target - last_u)/u_target > eps:
			last_x = xguess
...
# Set up plot
pylab.semilogx(t_array, [zmax]*N, color= 'k', linestyle = "dashed", label = "top of vessel")
pylab.semilogx(t_array, z_covered, color='b', marker="o", label = "water level")
pylab.semilogx(t_array, z_uncovered, color='r', marker="x")
pylab.semilogx(t_array, [ztop]*N, color= 'g', label = "top of core")
